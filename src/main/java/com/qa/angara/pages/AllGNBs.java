...
public class AllGNBs 
{
	...
	public static void main(String[] args) throws InterruptedException, TimeoutException {
		...
		Locator allGNBs = page.locator(".Sds-hQnKUn.MPEnz a");
		int countOfGNBHeader = allGNBs.count();
		int i;
		for(i=0; i<countOfGNBHeader;i++)
		{
			String gnbHeaders = allGNBs.nth(i).textContent();
			System.out.println(gnbHeaders);
			allGNBs.nth(i).hover();
			page.waitForLoadState(LoadState.NETWORKIDLE);
			Locator gnbLinks = page.locator(".sub-nav-options ul li a");
//			IntStream.range(40, gnbLinks.count()).forEach(j -> 
//			{
			int countOfGnbLinks=gnbLinks.count();
			for(int j=30;j<countOfGnbLinks;j++)
			{
				String link = gnbLinks.nth(j).getAttribute("href");
				...
				else if(link.startsWith("/l/custom-name-necklaces"))
				{
					continue;				
				}
				else 
				...
				if(j<countOfGnbLinks) 
				{
					allGNBs.nth(i).hover();
				}
				else if(j==countOfGnbLinks)
				{
